# Route → Service (ye code) → Repository → Database
# 👉 Ye layer decide karti hai:

# kab notification banani hai
# duplicate avoid karna hai
# security checks (403 / 404)
# response format
...
async def notify_like(db: AsyncSession, post_owner_id: int, liker_id: int, post_id: int):
    ...
    if post_owner_id == liker_id:
        return None
        # Apni post like ki → notification nahi ❌
...
async def notify_follow(db: AsyncSession, user_id: int, follower_id: int):
    ...
    return notification
    # 👉 Jab koi kisi ko follow karta hai
...
async def get_user_notifications_with_actor(db: AsyncSession, user_id: int, skip: int = 0, limit: int = 10):
    ...
    get_user_notifications_with_actor()

    # 👉 Ye function next level feature 🔥 hai

    # Kya karta hai:
    # notifications fetch karta hai
    # unread count nikalta hai
    # har notification me actor ka username add karta hai
    ...
    for notif in notifications:
        ...
        # 👉 Har notification ke liye DB hit 😬
...
async def read_notification(db: AsyncSession, notification_id: int, user_id: int):
    # 👉 Single notification read mark karta hai
